...
# Source functions
source("functions.R")
f <- ('./laserquestR/data/raw_data/UVAX_A4_01C.CSV')


data_dir <- "c:/github/laserquestR/data/raw_data/"
...
#df3 <- m1[,c(1,2,3,5)]
# m.1 <- normalize_pcl_one(m1)
# m.2 <- normalize_pcl_two(m.1)
# m.3 <- normalize_pcl_three(m.2)

# df3.vai <- calc_vai(m.3)
# 
# x11()
# plot(df3.vai$vai)

# df3$density <- df3$bin.hits / df3$can.hits
# 
# df3 <- df3[,c(1,2,5)]
# df3 <- m.tall.46[with(df3, order(xbin, zbin)),]
# 
# df3.vai <- calc_tls_vai(df3)
# df3.vai <- calc_tls_mean_leaf_ht(df3.vai)
# calc_tls_std_bin(df3.vai)
# 
# 
# 
